#!/usr/bin/env python3
"""Generate apply-ready patch outputs for source-reconcile gate waiver changes."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Prepare apply-ready patch outputs for source-reconcile gate waiver changes."
    )
...
def clear_prior_apply_artifacts(artifact_dir: Path) -> None:
    for pattern in ("apply-action-*.target.json", "apply-action-*.patch", "apply-action-*.review.json"):
        for candidate in artifact_dir.glob(pattern):
            remove_if_exists(candidate)
    remove_if_exists(artifact_dir / "apply.json")
    remove_if_exists(artifact_dir / "apply.md")
...
def render_waiver_apply_markdown(waiver_payload: dict) -> str:
    lines = [
        f"# Source Reconcile Gate Waiver Apply Pack: {waiver_payload.get('id', '')}",
        "",
...
def build_update_apply_action(action_preview: dict) -> tuple[dict, str, str]:
    ...
    return (
        {
            ...
            "candidate_policy_ids": action_preview.get("candidate_policy_ids", []),
            "candidate_report_scope": action_preview.get("candidate_report_scope", {}),
            "candidate_action": action_preview.get("candidate_action", {}),
            "candidate_finding_codes": action_preview.get("candidate_finding_codes", []),
...
def build_apply_payload(
    ...
    *,
    ...
    preview_payload = preview_source_reconcile_gate_waiver_execution.build_preview_payload(
        history_path,
    ...
    combined_patch_path = output_dir / "source-reconcile-gate-waiver-apply.patch"
    remove_if_exists(combined_patch_path)
    ...
    for waiver_preview in preview_payload.get("waivers", []):
        ...
        if artifact_dir is not None:
            clear_prior_apply_artifacts(artifact_dir)
        ...
        for index, action_preview in enumerate(waiver_preview.get("action_previews", []), start=1):
            ...
            if mode == "draft_update":
                action_payload, target_text, patch_text = build_update_apply_action(action_preview)
    ...
    return {
        ...
        "artifact_root": preview_payload.get("artifact_root", ""),
...
def render_markdown(payload: dict) -> str:
    lines = [
        "# Installed Baseline History Waiver Source Reconcile Gate Waiver Apply Pack",
        "",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json:
        ...
        print("Installed baseline history waiver source reconcile gate waiver apply pack:")
