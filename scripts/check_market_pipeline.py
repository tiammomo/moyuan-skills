...
def main(argv: list[str] | None = None) -> int:
    ...
    if "Installed Baseline History Waiver Source Reconcile" not in healthy_source_reconcile_markdown.read_text(encoding="utf-8"):
        ...
        return 1
    history_waiver_source_reconcile_execute_output = require_success(
        "execute installed baseline history waiver source reconcile",
        [
            "scripts/skills_market.py",
    ...
    history_waiver_source_reconcile_execute_payload = json.loads(history_waiver_source_reconcile_execute_output)
    if history_waiver_source_reconcile_execute_payload.get("passes") is not True or history_waiver_source_reconcile_execute_payload.get("action_count") != 0:
        print("ERROR: healthy waiver source reconcile execute should report zero execution actions")
        return 1
    healthy_source_reconcile_execute_json = history_waiver_execute_dir / "source-reconcile-execute-summary.json"
    healthy_source_reconcile_execute_markdown = history_waiver_execute_dir / "source-reconcile-execute-summary.md"
    if not healthy_source_reconcile_execute_json.is_file() or not healthy_source_reconcile_execute_markdown.is_file():
        print("ERROR: healthy waiver source reconcile execute should write summary artifacts")
        return 1
    ...
    history_waiver_source_reconcile_stage_payload = json.loads(history_waiver_source_reconcile_stage_output)
    if history_waiver_source_reconcile_stage_payload.get("staged_update_count") != 1 or history_waiver_source_reconcile_stage_payload.get("blocked_action_count") != 0:
        print("ERROR: tampered waiver source reconcile stage should emit one staged restore-target action without safety failures")
        return 1
    staged_reconcile_actions = history_waiver_source_reconcile_stage_payload.get("actions", [])
    if len(staged_reconcile_actions) != 1:
        print("ERROR: tampered waiver source reconcile stage should record exactly one staged action")
        return 1
    staged_reconcile_target_value = str(staged_reconcile_actions[0].get("stage_path", "")).strip()
    if not staged_reconcile_target_value:
        print("ERROR: staged source reconcile action should expose its staged target path")
        return 1
    staged_reconcile_target = ROOT / staged_reconcile_target_value
    if not staged_reconcile_target.is_file():
        print("ERROR: source reconcile staging should materialize the staged restore target under the staging root")
        return 1
    ...
    if "# Installed Baseline History Waiver Source Reconcile Gate Waiver Execution Drafts" not in source_reconcile_gate_waiver_execution_markdown.read_text(encoding="utf-8"):
        ...
        return 1
    source_reconcile_gate_waiver_preview_json = output_root / "snapshots" / "source-reconcile-gate-waiver-preview.json"
    source_reconcile_gate_waiver_preview_result = run_python(
        [
            "scripts/skills_market.py",
            ...
            repo_relative_path(history_waiver_source_reconcile_write_summary_json),
            "--output-path",
            repo_relative_path(source_reconcile_gate_waiver_preview_json),
            "--json",
            "--strict",
        ]
    )
    if source_reconcile_gate_waiver_preview_result.returncode == 0:
        print("ERROR: source-reconcile gate waiver preview should fail in strict mode when review previews exist")
        if source_reconcile_gate_waiver_preview_result.stdout.strip():
            print(source_reconcile_gate_waiver_preview_result.stdout.strip())
        if source_reconcile_gate_waiver_preview_result.stderr.strip():
            print(source_reconcile_gate_waiver_preview_result.stderr.strip())
        return 1
    source_reconcile_gate_waiver_preview_payload = load_json(source_reconcile_gate_waiver_preview_json)
    if source_reconcile_gate_waiver_preview_payload.get("preview_count") != 4:
        print("ERROR: source-reconcile gate waiver preview should emit one preview action for each execution action")
        return 1
    if source_reconcile_gate_waiver_preview_payload.get("draft_preview_count") != 3:
        print("ERROR: source-reconcile gate waiver preview should create three draft previews")
        return 1
    if source_reconcile_gate_waiver_preview_payload.get("review_preview_count") != 1:
        print("ERROR: source-reconcile gate waiver preview should create one review preview for the policy mismatch action")
        return 1
    source_reconcile_gate_waiver_preview_by_id = {
        item.get("id"): item
        for item in source_reconcile_gate_waiver_preview_payload.get("waivers", [])
        if isinstance(item, dict)
    }
    if source_reconcile_gate_waiver_preview_by_id.get("approved-expired-release-downsize-source-drift", {}).get("action_previews") not in ([], None):
        print("ERROR: healthy built-in source-reconcile gate waiver should not require preview actions")
        return 1
    for waiver_id in ("expired-source-drift", "stale-blocked-execution", "unmatched-drift", "policy-mismatch-drift"):
        waiver_preview = source_reconcile_gate_waiver_preview_by_id.get(waiver_id, {})
        preview_path = str(waiver_preview.get("preview_path", "")).strip()
        preview_markdown_path = str(waiver_preview.get("preview_markdown_path", "")).strip()
        if not preview_path or not (ROOT / preview_path).is_file():
            print("ERROR: source-reconcile gate waiver preview should materialize per-waiver preview JSON artifacts")
            return 1
        if not preview_markdown_path or not (ROOT / preview_markdown_path).is_file():
            print("ERROR: source-reconcile gate waiver preview should materialize per-waiver preview Markdown artifacts")
            return 1
        if not waiver_preview.get("action_previews"):
            print("ERROR: failing source-reconcile gate waivers should carry action preview entries")
            return 1
    if source_reconcile_gate_waiver_preview_by_id.get("expired-source-drift", {}).get("action_previews", [{}])[0].get("draft_strategy") != "renew":
        print("ERROR: expired source-reconcile gate waiver preview should expose the renewal draft strategy")
        return 1
    if source_reconcile_gate_waiver_preview_by_id.get("stale-blocked-execution", {}).get("action_previews", [{}])[0].get("draft_strategy") != "replace":
        print("ERROR: stale source-reconcile gate waiver preview should expose the replacement draft strategy")
        return 1
    if source_reconcile_gate_waiver_preview_by_id.get("unmatched-drift", {}).get("action_previews", [{}])[0].get("draft_strategy") != "retarget":
        print("ERROR: unmatched source-reconcile gate waiver preview should expose the retarget draft strategy")
        return 1
    policy_mismatch_preview_changes = source_reconcile_gate_waiver_preview_by_id.get("policy-mismatch-drift", {}).get("action_previews", [{}])[0].get("changes", [])
    if {item.get("change_type") for item in policy_mismatch_preview_changes if isinstance(item, dict)} != {"review"}:
        print("ERROR: policy-mismatch source-reconcile gate waiver preview should stay as a review-only change summary")
        return 1
    source_reconcile_gate_waiver_preview_markdown = history_waiver_execute_write_updates_dir / "source-reconcile-gate-waiver-preview-summary.md"
    if "# Installed Baseline History Waiver Source Reconcile Gate Waiver Preview" not in source_reconcile_gate_waiver_preview_markdown.read_text(encoding="utf-8"):
        print("ERROR: source-reconcile gate waiver preview Markdown output should contain the preview heading")
        return 1
    source_reconcile_gate_waiver_apply_json = output_root / "snapshots" / "source-reconcile-gate-waiver-apply.json"
    source_reconcile_gate_waiver_apply_result = run_python(
        [
            "scripts/skills_market.py",
            ...
            repo_relative_path(history_waiver_source_reconcile_write_summary_json),
            "--output-path",
            repo_relative_path(source_reconcile_gate_waiver_apply_json),
            "--json",
            "--strict",
        ]
    )
    if source_reconcile_gate_waiver_apply_result.returncode == 0:
        print("ERROR: source-reconcile gate waiver apply pack should fail in strict mode when apply actions exist")
        if source_reconcile_gate_waiver_apply_result.stdout.strip():
            print(source_reconcile_gate_waiver_apply_result.stdout.strip())
        if source_reconcile_gate_waiver_apply_result.stderr.strip():
            print(source_reconcile_gate_waiver_apply_result.stderr.strip())
        return 1
    source_reconcile_gate_waiver_apply_payload = load_json(source_reconcile_gate_waiver_apply_json)
    if source_reconcile_gate_waiver_apply_payload.get("action_count") != 4:
        print("ERROR: source-reconcile gate waiver apply pack should emit one apply action for each preview action")
        return 1
    if source_reconcile_gate_waiver_apply_payload.get("patch_count") != 3:
        print("ERROR: source-reconcile gate waiver apply pack should emit three patch files for the draft-backed waivers")
        return 1
    if source_reconcile_gate_waiver_apply_payload.get("update_patch_count") != 3:
        print("ERROR: source-reconcile gate waiver apply pack should classify the three generated patches as update patches")
        return 1
    if source_reconcile_gate_waiver_apply_payload.get("manual_review_count") != 1:
        print("ERROR: source-reconcile gate waiver apply pack should keep the policy-mismatch follow-up as manual review")
        return 1
    combined_source_reconcile_apply_patch = str(source_reconcile_gate_waiver_apply_payload.get("combined_patch_path", "")).strip()
    if not combined_source_reconcile_apply_patch or not (ROOT / combined_source_reconcile_apply_patch).is_file():
        print("ERROR: source-reconcile gate waiver apply pack should materialize a combined patch bundle")
        return 1
    source_reconcile_gate_waiver_apply_by_id = {
        item.get("id"): item
        for item in source_reconcile_gate_waiver_apply_payload.get("waivers", [])
        if isinstance(item, dict)
    }
    for waiver_id in ("expired-source-drift", "stale-blocked-execution", "unmatched-drift"):
        waiver_apply = source_reconcile_gate_waiver_apply_by_id.get(waiver_id, {})
        apply_actions = waiver_apply.get("apply_actions", [])
        if len(apply_actions) != 1:
            print("ERROR: each draft-backed source-reconcile gate waiver should expose exactly one apply action in the smoke workflow")
            return 1
        action = apply_actions[0]
        if not str(action.get("target_path", "")).strip() or not (ROOT / str(action.get("target_path", "")).strip()).is_file():
            print("ERROR: source-reconcile gate waiver apply updates should materialize target JSON artifacts")
            return 1
        if not str(action.get("patch_path", "")).strip() or not (ROOT / str(action.get("patch_path", "")).strip()).is_file():
            print("ERROR: source-reconcile gate waiver apply updates should materialize patch artifacts")
            return 1
    policy_mismatch_apply_actions = source_reconcile_gate_waiver_apply_by_id.get("policy-mismatch-drift", {}).get("apply_actions", [])
    if len(policy_mismatch_apply_actions) != 1:
        print("ERROR: policy-mismatch source-reconcile gate waiver should expose exactly one apply action in the smoke workflow")
        return 1
    policy_mismatch_apply_action = policy_mismatch_apply_actions[0]
    if str(policy_mismatch_apply_action.get("patch_path", "")).strip():
        print("ERROR: policy-mismatch source-reconcile gate waiver should stay review-only and not emit a patch file")
        return 1
    review_artifact_path = str(policy_mismatch_apply_action.get("review_artifact_path", "")).strip()
    if not review_artifact_path or not (ROOT / review_artifact_path).is_file():
        print("ERROR: policy-mismatch source-reconcile gate waiver should materialize a manual review artifact")
        return 1
    source_reconcile_gate_waiver_apply_markdown = history_waiver_execute_write_updates_dir / "source-reconcile-gate-waiver-apply-summary.md"
    ...
    for waiver_id in ("expired-source-drift", "stale-blocked-execution", "unmatched-drift"):
        ...
        if mirrored_source.read_text(encoding="utf-8") != target_artifact.read_text(encoding="utf-8"):
            ...
            return 1
    source_reconcile_gate_waiver_apply_verify_stage_json = output_root / "snapshots" / "source-reconcile-gate-waiver-apply-verify-stage.json"
    source_reconcile_gate_waiver_apply_verify_stage_markdown = output_root / "snapshots" / "source-reconcile-gate-waiver-apply-verify-stage.md"
    source_reconcile_gate_waiver_apply_verify_stage_output = require_success(
        "verify staged source-reconcile gate waiver apply execution",
        [
            "scripts/skills_market.py",
            ...
            repo_relative_path(source_reconcile_gate_waiver_apply_execute_stage_json),
            "--output-path",
            repo_relative_path(source_reconcile_gate_waiver_apply_verify_stage_json),
            "--markdown-path",
            repo_relative_path(source_reconcile_gate_waiver_apply_verify_stage_markdown),
            "--json",
            "--strict",
        ],
    )
    source_reconcile_gate_waiver_apply_verify_stage_payload = json.loads(source_reconcile_gate_waiver_apply_verify_stage_output)
    if source_reconcile_gate_waiver_apply_verify_stage_payload.get("staged_target_match_count") != 3 or source_reconcile_gate_waiver_apply_verify_stage_payload.get("manual_review_count") != 1:
        print("ERROR: staged source-reconcile gate waiver apply verification should confirm three staged targets and one manual review hold")
        return 1
    if source_reconcile_gate_waiver_apply_verify_stage_payload.get("drift_count") != 0 or source_reconcile_gate_waiver_apply_verify_stage_payload.get("blocked_count") != 0:
        print("ERROR: staged source-reconcile gate waiver apply verification should not report drift or blocked findings in the healthy smoke workflow")
        return 1
    if "# Installed Baseline History Waiver Source Reconcile Gate Waiver Apply Verification" not in source_reconcile_gate_waiver_apply_verify_stage_markdown.read_text(encoding="utf-8"):
        print("ERROR: source-reconcile gate waiver apply verification Markdown output should contain the verification heading")
        return 1
    source_reconcile_gate_waiver_apply_verify_write_json = output_root / "snapshots" / "source-reconcile-gate-waiver-apply-verify-write.json"
    source_reconcile_gate_waiver_apply_verify_write_output = require_success(
        "verify written source-reconcile gate waiver apply execution",
        [
            "scripts/skills_market.py",
            "verify-installed-history-waiver-source-reconcile-waiver-apply",
            repo_relative_path(promotion_history_json),
            ...
            repo_relative_path(source_reconcile_gate_waiver_apply_execute_write_json),
            "--output-path",
            repo_relative_path(source_reconcile_gate_waiver_apply_verify_write_json),
            "--json",
            "--strict",
        ],
    )
    source_reconcile_gate_waiver_apply_verify_write_payload = json.loads(source_reconcile_gate_waiver_apply_verify_write_output)
    ...
    tampered_source_reconcile_gate_apply_source.write_text(
        tampered_source_reconcile_gate_apply_source.read_text(encoding="utf-8").replace(
            "TODO: refresh the approval rationale",
            "tampered source-reconcile gate waiver apply verification",
            1,
        ),
        encoding="utf-8",
    )
    source_reconcile_gate_waiver_apply_verify_tampered_result = run_python(
        [
            "scripts/skills_market.py",
            "verify-installed-history-waiver-source-reconcile-waiver-apply",
            repo_relative_path(promotion_history_json),
    ...
    if source_reconcile_gate_waiver_apply_verify_tampered_result.returncode == 0:
        print("ERROR: source-reconcile gate waiver apply verification should fail in strict mode after a written target drifts")
        if source_reconcile_gate_waiver_apply_verify_tampered_result.stdout.strip():
            print(source_reconcile_gate_waiver_apply_verify_tampered_result.stdout.strip())
        if source_reconcile_gate_waiver_apply_verify_tampered_result.stderr.strip():
            print(source_reconcile_gate_waiver_apply_verify_tampered_result.stderr.strip())
        return 1
    ...
    if source_reconcile_gate_waiver_apply_verify_tampered_payload.get("drift_count") != 1:
        ...
        return 1
    source_reconcile_gate_waiver_apply_release_gate_tampered_result = run_python(
        [
            "scripts/skills_market.py",
            "gate-installed-history-waiver-source-reconcile-waiver-apply",
            repo_relative_path(promotion_history_json),
    ...
    if source_reconcile_gate_waiver_apply_release_gate_tampered_result.returncode == 0:
        print("ERROR: release apply gate should fail in strict mode after a written target drifts")
        if source_reconcile_gate_waiver_apply_release_gate_tampered_result.stdout.strip():
            print(source_reconcile_gate_waiver_apply_release_gate_tampered_result.stdout.strip())
        if source_reconcile_gate_waiver_apply_release_gate_tampered_result.stderr.strip():
            print(source_reconcile_gate_waiver_apply_release_gate_tampered_result.stderr.strip())
        return 1
    source_reconcile_gate_waiver_apply_release_gate_tampered_payload = json.loads(source_reconcile_gate_waiver_apply_release_gate_tampered_result.stdout)
    if source_reconcile_gate_waiver_apply_release_gate_tampered_payload.get("active_finding_count") != 2:
        print("ERROR: release apply gate should report two active findings for the tampered workflow")
        return 1
    source_reconcile_gate_waiver_apply_release_gate_waived_output = require_success(
        "gate tampered source-reconcile gate waiver apply workflow with a targeted apply-gate waiver",
        [
            "scripts/skills_market.py",
            "gate-installed-history-waiver-source-reconcile-waiver-apply",
            repo_relative_path(promotion_history_json),
            ...
            repo_relative_path(unmatched_source_reconcile_gate_waiver_path),
            "--apply-gate-waiver",
            "approved-expired-source-reconcile-gate-waiver-apply-drift",
            "--policy",
            "source-reconcile-waiver-apply-release-gate",
            "--output-dir",
            repo_relative_path(history_waiver_execute_write_updates_dir),
    ...
    source_reconcile_gate_waiver_apply_release_gate_waived_payload = json.loads(source_reconcile_gate_waiver_apply_release_gate_waived_output)
    if source_reconcile_gate_waiver_apply_release_gate_waived_payload.get("passes") is not True or source_reconcile_gate_waiver_apply_release_gate_waived_payload.get("report_state") != "drifted":
        print("ERROR: release apply gate with a targeted waiver should allow the tampered workflow while preserving report_state=drifted")
        return 1
    if source_reconcile_gate_waiver_apply_release_gate_waived_payload.get("active_finding_count") != 0 or source_reconcile_gate_waiver_apply_release_gate_waived_payload.get("waived_finding_count") != 2:
        print("ERROR: release apply gate waiver should clear both active findings and record them as waived")
        return 1
    source_reconcile_gate_waiver_apply_temp_dir = fixture_root / "source-reconcile-gate-waiver-apply-waivers"
    source_reconcile_gate_waiver_apply_temp_dir.mkdir(parents=True, exist_ok=True)
    expired_source_reconcile_gate_waiver_apply_waiver_path = source_reconcile_gate_waiver_apply_temp_dir / "expired-apply-drift.json"
    expired_source_reconcile_gate_waiver_apply_waiver_path.write_text(
        json.dumps(
            {
                "waiver_version": 1,
                "id": "expired-source-reconcile-gate-waiver-apply-drift-temp",
                "title": "Expired Source Reconcile Gate Waiver Apply Drift",
                "description": "An expired apply-gate waiver kept only so the smoke test can confirm that waiver audits catch old apply-drift approvals.",
                "policy_id": "source-reconcile-waiver-apply-release-gate",
                "match": {
                    "finding_codes": [
                        "disallowed_report_state",
                        "verification_drift"
                    ...
                    "target_root_suffix": "source-reconcile-gate-waiver-apply-write-root",
                    "action_waiver_ids": [
                        "expired-source-drift"
                    ],
                    "action_codes": [
                        "renew_or_remove"
                    ]
                ...
                "approval": {
                    "approved_by": "Moyuan Governance Review",
                    "approved_at": "2026-03-20",
                    "reason": "This expired fixture exists only so the smoke test can confirm that apply-gate waiver audits catch old approvals."
                },
                "expires_on": "2026-03-21"
            },
            indent=2,
    ...
    stale_source_reconcile_gate_waiver_apply_waiver_path = source_reconcile_gate_waiver_apply_temp_dir / "stale-blocked-execution.json"
    stale_source_reconcile_gate_waiver_apply_waiver_path.write_text(
        json.dumps(
            {
                "waiver_version": 1,
                "id": "stale-source-reconcile-gate-waiver-apply-drift",
                "title": "Stale Source Reconcile Gate Waiver Apply Drift",
                "description": "A stale apply-gate waiver that still points at the report scope but no longer matches any active finding code.",
                "policy_id": "source-reconcile-waiver-apply-release-gate",
                "match": {
                    "finding_codes": [
                        "blocked_execution"
                ...
                "approval": {
                    "approved_by": "Moyuan Governance Review",
                    "approved_at": "2026-03-26",
                    "reason": "This fixture exists only so the smoke test can confirm that apply-gate waiver audits catch stale approvals."
                },
                "expires_on": "2026-12-31"
            },
            indent=2,
    ...
    unmatched_source_reconcile_gate_waiver_apply_waiver_path = source_reconcile_gate_waiver_apply_temp_dir / "unmatched-drift.json"
    unmatched_source_reconcile_gate_waiver_apply_waiver_path.write_text(
        json.dumps(
            {
                "waiver_version": 1,
                "id": "unmatched-source-reconcile-gate-waiver-apply-drift",
                "title": "Unmatched Source Reconcile Gate Waiver Apply Drift",
                "description": "An apply-gate waiver that no longer matches the current report scope because it points at the wrong target root suffix.",
                "policy_id": "source-reconcile-waiver-apply-release-gate",
                "match": {
                    "finding_codes": [
                        "disallowed_report_state",
                ...
                "approval": {
                    "approved_by": "Moyuan Governance Review",
                    "approved_at": "2026-03-26",
                    "reason": "This fixture exists only so the smoke test can confirm that apply-gate waiver audits catch unmatched approvals."
                },
                "expires_on": "2026-12-31"
            },
            indent=2,
    ...
    policy_mismatch_source_reconcile_gate_waiver_apply_waiver_path = source_reconcile_gate_waiver_apply_temp_dir / "policy-mismatch-drift.json"
    policy_mismatch_source_reconcile_gate_waiver_apply_waiver_path.write_text(
        json.dumps(
            {
                "waiver_version": 1,
                "id": "policy-mismatch-source-reconcile-gate-waiver-apply-drift",
                "title": "Policy Mismatch Source Reconcile Gate Waiver Apply Drift",
                "description": "An apply-gate waiver that now only matches under the strict release policy, so audit should flag the stored review-handoff policy as out of date.",
                "policy_id": "source-reconcile-waiver-apply-review-handoff",
                "match": {
                    "finding_codes": [
                        "disallowed_report_state",
                        "verification_drift"
                ...
                "approval": {
                    "approved_by": "Moyuan Governance Review",
                    "approved_at": "2026-03-26",
                    "reason": "This fixture exists only so the smoke test can confirm that apply-gate waiver audits catch policy mismatches."
                },
                "expires_on": "2026-12-31"
            },
            indent=2,
    ...
    source_reconcile_gate_waiver_apply_audit_result = run_python(
        [
            "scripts/skills_market.py",
            ...
            "approved-expired-source-reconcile-gate-waiver-apply-drift",
            "--apply-gate-waiver",
            repo_relative_path(expired_source_reconcile_gate_waiver_apply_waiver_path),
            "--apply-gate-waiver",
            repo_relative_path(stale_source_reconcile_gate_waiver_apply_waiver_path),
            "--apply-gate-waiver",
            repo_relative_path(unmatched_source_reconcile_gate_waiver_apply_waiver_path),
            "--apply-gate-waiver",
            repo_relative_path(policy_mismatch_source_reconcile_gate_waiver_apply_waiver_path),
            "--output-dir",
            repo_relative_path(history_waiver_execute_write_updates_dir),
            "--target-root",
            repo_relative_path(source_reconcile_gate_waiver_apply_write_root),
    ...
    if source_reconcile_gate_waiver_apply_audit_result.returncode == 0:
        print("ERROR: source-reconcile gate waiver apply waiver audit should fail in strict mode when expired, unmatched, stale, or policy-mismatch waivers exist")
        if source_reconcile_gate_waiver_apply_audit_result.stdout.strip():
            print(source_reconcile_gate_waiver_apply_audit_result.stdout.strip())
        if source_reconcile_gate_waiver_apply_audit_result.stderr.strip():
            print(source_reconcile_gate_waiver_apply_audit_result.stderr.strip())
        return 1
    source_reconcile_gate_waiver_apply_audit_payload = json.loads(source_reconcile_gate_waiver_apply_audit_result.stdout)
    if source_reconcile_gate_waiver_apply_audit_payload.get("waiver_count") != 5 or source_reconcile_gate_waiver_apply_audit_payload.get("finding_count") != 4:
        print("ERROR: source-reconcile gate waiver apply waiver audit should inspect five waivers and emit four findings")
        return 1
    if source_reconcile_gate_waiver_apply_audit_payload.get("expired_count") != 1 or source_reconcile_gate_waiver_apply_audit_payload.get("unmatched_count") != 1 or source_reconcile_gate_waiver_apply_audit_payload.get("stale_count") != 1 or source_reconcile_gate_waiver_apply_audit_payload.get("policy_mismatch_count") != 1:
        print("ERROR: source-reconcile gate waiver apply waiver audit should classify expired, unmatched, stale, and policy-mismatch findings exactly once")
        return 1
    source_reconcile_gate_waiver_apply_audit_results = {
        entry.get("id"): entry
        for entry in source_reconcile_gate_waiver_apply_audit_payload.get("waivers", [])
        if isinstance(entry, dict)
    }
    if source_reconcile_gate_waiver_apply_audit_results.get("approved-expired-source-reconcile-gate-waiver-apply-drift", {}).get("passes") is not True:
        print("ERROR: built-in apply-gate waiver should remain healthy in the waiver audit smoke flow")
        return 1
    if source_reconcile_gate_waiver_apply_audit_results.get("policy-mismatch-source-reconcile-gate-waiver-apply-drift", {}).get("matched_policy_ids") != ["source-reconcile-waiver-apply-release-gate"]:
        print("ERROR: policy-mismatch apply-gate waiver audit should point reviewers at the release-gate policy")
        return 1
