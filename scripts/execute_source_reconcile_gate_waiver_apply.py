...
def staged_target_path(waiver_id: str, action: dict, source_path: Path, stage_dir: Path) -> Path:
    ...
    return stage_dir / "targets" / normalized_waiver_id / f"{action_code}-{source_path.name}"
    ...
    *,
    ...
    if target_path:
        ...
        if stage_dir is not None:
            staged_path = staged_target_path(waiver_id, action, actual_target_path, stage_dir).resolve()
