#!/usr/bin/env python3
"""Evaluate source-reconcile gate waiver apply reports as a reusable gate."""
...
DEFAULT_ALLOWED_STATES = ("verified", "no_apply_actions")
APPLY_GATE_POLICIES_DIR = ROOT / "governance" / "source-reconcile-gate-waiver-apply-policies"
APPLY_GATE_WAIVERS_DIR = ROOT / "governance" / "source-reconcile-gate-waiver-apply-waivers"
APPLY_REPORT_STATES = (
    "needs_execution",
    "drifted",
...
def iter_policy_paths() -> list[Path]:
    return sorted(APPLY_GATE_POLICIES_DIR.glob("*.json"))


def iter_waiver_paths() -> list[Path]:
    return sorted(APPLY_GATE_WAIVERS_DIR.glob("*.json"))
...
def validate_policy_payload(path: Path) -> tuple[dict, list[str]]:
    ...
    if (
        ...
        invalid_states = sorted(
            {item.strip() for item in allowed_states if item.strip() not in APPLY_REPORT_STATES}
        )
        if invalid_states:
            errors.append(f"{label}: unsupported allowed_states values: {', '.join(invalid_states)}")
    ...
    if not isinstance(match, dict):
        ...
        if (
            ...
            invalid_codes = sorted(
                {item.strip() for item in finding_codes if item.strip() not in APPLY_GATE_FINDING_CODES}
            )
            if invalid_codes:
        ...
        if report_states is not None:
            ...
            if (
                ...
                invalid_states = sorted(
                    {item.strip() for item in report_states if item.strip() not in APPLY_REPORT_STATES}
                )
                if invalid_states:
...
def resolve_policy_reference(token: str) -> tuple[dict, Path]:
    ...
    raise SystemExit(f"source-reconcile gate waiver apply policy not found: {token}")
...
def resolve_waiver_reference(token: str) -> tuple[dict, Path]:
    ...
    raise SystemExit(f"source-reconcile gate waiver apply waiver not found: {token}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Evaluate source-reconcile gate waiver apply reports as a reusable gate."
    )
    parser.add_argument("history", type=Path, help="Baseline history JSON file.")
    parser.add_argument("--policy", help="Named policy id or JSON file path for reusable apply gate rules.")
    parser.add_argument(
        "--waiver",
        action="append",
        default=[],
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Named source-reconcile gate waiver id or JSON file path used when building the apply report context.",
    )
    parser.add_argument(
        "--apply-gate-waiver",
        action="append",
    ...
    parser.add_argument("--output-dir", type=Path, help="Directory containing apply artifacts and receiving gate summaries.")
...
def build_gate_payload(
    ...
    *,
        ...
        findings.append(
            {
                ...
                "message": "apply handoff is incomplete because one or more workflow summaries are missing",
            }
        )
    ...
    execution_summary = report_payload.get("apply_execution", {})
    if defaults["fail_on_blocked_execution"] and execution_summary.get("blocked_action_count", 0) > 0:
        findings.append(
            {
                "code": "blocked_execution",
                "message": "apply execution contains blocked actions that must be resolved before promotion",
            }
        )

    verification_summary = report_payload.get("apply_verification", {})
    if defaults["fail_on_pending_verification"] and verification_summary.get("pending_count", 0) > 0:
        findings.append(
            {
                "code": "pending_verification",
                "message": "apply verification still has pending actions and cannot be treated as complete",
            }
        )
        ...
        findings.append(
            {
                ...
                "message": "apply verification contains blocked actions and requires follow-up",
            }
        )
        ...
        findings.append(
            {
                ...
                "message": "apply verification detected drift after execution",
            }
        )
    ...
    payload = {
        ...
        "report_summary_path": display_path(output_dir / "source-reconcile-gate-waiver-apply-report-summary.json"),
        "apply_summary_path": report_payload.get("apply_summary_path", ""),
        "apply_execute_summary_path": report_payload.get("apply_execute_summary_path", ""),
        "apply_verify_summary_path": report_payload.get("apply_verify_summary_path", ""),
        "finding_count": len(findings),
        "findings": findings,
        "report": report_payload,
    }
    apply_gate_waivers = resolve_requested_waivers(apply_gate_waiver_tokens)
    ...
    payload["apply_gate_waivers"] = waiver_summaries
    payload["requested_apply_gate_waiver_count"] = len(waiver_summaries)
    payload["matched_apply_gate_waiver_count"] = sum(1 for item in waiver_summaries if item.get("matched"))
...
def render_markdown(payload: dict) -> str:
    lines = [
        "# Installed Baseline History Waiver Source Reconcile Gate Waiver Apply Gate",
        "",
    ...
    apply_gate_waivers = payload.get("apply_gate_waivers", [])
    if not apply_gate_waivers:
        lines.append("- No apply gate waivers requested.")
    else:
        for waiver in apply_gate_waivers:
            lines.append(
...
def main(argv: list[str] | None = None) -> int:
    ...
    output_dir = resolve_repo_path(args.output_dir) if args.output_dir else report_source_reconcile_gate_waiver_apply.DEFAULT_OUTPUT_DIR
    ...
    payload = build_gate_payload(
        ...
        allowed_states=allowed_states,
        apply_gate_waiver_tokens=args.apply_gate_waiver,
    )

    report_summary_json_path = output_dir / "source-reconcile-gate-waiver-apply-report-summary.json"
    report_summary_markdown_path = output_dir / "source-reconcile-gate-waiver-apply-report-summary.md"
    write_text(report_summary_json_path, render_json_payload(payload["report"]))
    write_text(report_summary_markdown_path, report_source_reconcile_gate_waiver_apply.render_markdown(payload["report"]))

    summary_json_path = (
        resolve_repo_path(args.output_path)
        if args.output_path
        else (output_dir / "source-reconcile-gate-waiver-apply-gate-summary.json")
    )
    summary_markdown_path = (
        resolve_repo_path(args.markdown_path)
        if args.markdown_path
        else (output_dir / "source-reconcile-gate-waiver-apply-gate-summary.md")
    )
    ...
    if args.json:
        ...
        print("Installed baseline history waiver source reconcile gate waiver apply gate:")
