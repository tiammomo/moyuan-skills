#!/usr/bin/env python3
"""Verify source-reconcile gate waiver apply execution results against reviewed apply artifacts."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Verify source-reconcile gate waiver apply execution results against reviewed apply artifacts."
    )
    ...
    parser.add_argument(
        "--strict",
        action="store_true",
        help="Return a non-zero exit code when verification drift is detected.",
    )
    return parser
...
def sha256_for_normalized_text_file(path: Path) -> str:
    ...
    *,
    ...
    if not execute_status:
        return build_pending_result(
            ...
            execute_status,
            "",
            "source-reconcile gate waiver apply action exists, but no execution record is available yet",
        )

    if execute_status == "blocked_manual_review":
        return {
            "waiver_id": action.get("waiver_id"),
            "action_code": action.get("action_code"),
            "mode": action.get("mode"),
            "execute_status": execute_status,
            "verification_path": str(execute_action.get("review_artifact_path", "")).strip(),
            "state": "manual_review",
    ...
    if execute_status.startswith("blocked"):
        return build_blocked_result(
            ...
            str(execute_action.get("message", "")).strip() or "source-reconcile gate waiver apply execution was blocked",
        )
        ...
        if stage_path_value:
            ...
            if stage_dir is None:
                return build_pending_result(
                    ...
                    execute_status,
                    "",
                    "staged apply verification requires a stage path or stage directory",
                )
            source_path = resolve_written_source_path(action, execute_action)
            stage_path = execute_source_reconcile_gate_waiver_apply.staged_target_path(
                str(action.get("waiver_id", "")).strip(),
                action,
                source_path,
                stage_dir,
            )
        current_sha = sha256_for_normalized_text_file(stage_path) if stage_path.is_file() else ""
        if stage_path.is_file() and current_sha == target_sha:
            state = "matches_staged_target"
            passes = True
            message = "staged apply target still matches the reviewed target artifact"
        elif not stage_path.is_file():
            state = "missing_staged_target"
            passes = False
            message = "staged apply target is missing"
        else:
            state = "drifted_staged_target"
            passes = False
            message = "staged apply target no longer matches the reviewed target artifact"
        return {
            "waiver_id": action.get("waiver_id"),
            "action_code": action.get("action_code"),
            "mode": action.get("mode"),
    ...
    if execute_status == "written":
        source_path = resolve_written_source_path(action, execute_action)
        current_sha = sha256_for_normalized_text_file(source_path) if source_path.is_file() else ""
        if source_path.is_file() and current_sha == target_sha:
            state = "matches_written_target"
            passes = True
            message = "written apply target still matches the reviewed target artifact"
        elif not source_path.is_file():
            state = "missing_written_target"
            passes = False
            message = "written apply target is missing"
        else:
            state = "drifted_after_write"
            passes = False
            message = "written apply target no longer matches the reviewed target artifact"
        return {
            "waiver_id": action.get("waiver_id"),
            "action_code": action.get("action_code"),
            "mode": action.get("mode"),
    ...
    return build_blocked_result(
        ...
        execute_status,
        "",
        f"unexpected execute status for apply update action: {execute_status}",
    )


def verify_delete_action(
    action: dict,
    execute_action: dict,
    *,
    target_root: Path,
    ...
    if not execute_status:
        return build_pending_result(
            ...
            execute_status,
            "",
            "source-reconcile gate waiver delete action exists, but no execution record is available yet",
        )
    ...
    if execute_status == "staged_delete":
        ...
        if stage_path.is_file():
            try:
                ...
                deletions = []
        source_path = resolve_written_source_path(action, execute_action)
        expected_entry = execute_source_reconcile_gate_waiver_apply.relative_stage_path(
            source_path,
            target_root,
        ).as_posix()
    ...
    if execute_status == "written_delete":
        ...
        if not source_path.exists():
            ...
            passes = True
            message = "written delete still matches the reviewed delete state"
            current_sha = ""
        else:
            state = "drifted_after_delete"
            passes = False
            message = "written delete no longer matches the reviewed delete state"
    ...
    return build_blocked_result(
        ...
        execute_status,
        "",
        f"unexpected execute status for apply delete action: {execute_status}",
    )
    ...
    *,
    ...
    if execute_status in {"", "blocked_manual_review"}:
        ...
        return {
            ...
            "mode": action.get("mode"),
            "execute_status": execute_status,
            "verification_path": str(action.get("review_artifact_path", "")).strip(),
            "state": "manual_review",
            "expected_state": "manual_review",
            "current_sha256": "",
            "passes": True,
    ...
    return build_blocked_result(
        ...
        execute_status,
        "",
        f"unexpected apply verification mode: {action.get('mode', '')}",
    )
...
def build_verification_payload(
    ...
    *,
    ...
    execute_payload, resolved_apply_execute_summary_path = load_apply_execute_payload(
        output_dir,
        apply_execute_summary_path,
    )
    ...
    for waiver in apply_payload.get("waivers", []):
        ...
        for action in waiver.get("apply_actions", []):
            if not isinstance(action, dict):
                continue
            action_with_waiver = dict(action)
            action_with_waiver["waiver_id"] = waiver.get("id")
            key = (
                str(waiver.get("id", "")).strip(),
    ...
    return {
        ...
        "apply_execute_summary_path": resolved_apply_execute_summary_path,
...
def render_markdown(payload: dict) -> str:
    lines = [
        "# Installed Baseline History Waiver Source Reconcile Gate Waiver Apply Verification",
        "",
    ...
    if not results:
        lines.append("- No source-reconcile gate waiver apply verification actions required.")
    else:
...
def main(argv: list[str] | None = None) -> int:
    ...
    summary_json_path = (
        ...
        if args.output_path
        else (output_dir / "source-reconcile-gate-waiver-apply-verify-summary.json")
    )
    summary_markdown_path = (
        resolve_repo_path(args.markdown_path)
        if args.markdown_path
        else (output_dir / "source-reconcile-gate-waiver-apply-verify-summary.md")
    )
    ...
    if args.json:
        ...
        print("Installed baseline history waiver source reconcile gate waiver apply verification:")
