#!/usr/bin/env python3
"""List reusable source-reconcile gate waiver apply policy profiles."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="List reusable source-reconcile gate waiver apply policies.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    policies, errors = check_source_reconcile_gate_waiver_apply_gate.load_policy_profiles()
    if errors:
        for error in errors:
            print(f"ERROR: {error}")
        return 1
    ...
    print("Source-reconcile gate waiver apply policies:")
