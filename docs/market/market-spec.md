...
1. `skill submission`
2. `submission review artifact`
3. `submission ingest receipt`
...
incoming/submissions/<publisher>/<skill>/<version>/ingest.json
...
- 基于 `schemas/skill-submission-review.schema.json` 写出 `review.json`
...
- 在 inbox 写出 `ingest.json` 作为 maintainer receipt
