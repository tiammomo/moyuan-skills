...
当前前端页面已经覆盖本地 market 的 `/skills`、`/bundles`、`/docs` 和作者工作台 `/studio`，但远端 registry browse 仍然以 CLI 和 backend API 为主，`/registries/*` 页面尚未落地。
...
- `apply-gate waiver` 适合在 policy 仍然保持严格的前提下，对个别已审批 drift 做显式例外，而不是把整条 gate 改成 review-handoff
