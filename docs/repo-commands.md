...
现在 docs 详情页里的 action panel 已经能对 allowlist 命令直接发起 backend job，并保留 recent runs / last-success、recent runs compare / filter，以及 artifact、stdout、stderr drilldown；但这里仍然是完整命令参考的权威来源，适合手动执行、排错和批量串联。
