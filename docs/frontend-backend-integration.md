...
- docs action panel 的 allowlist backend 执行、copy fallback、前置条件状态、recent runs compare / filter、last-success 回看，以及 artifact / stdout / stderr drilldown
...
- docs 上下文面板
- action panel 的命令、执行模式、顺序、前置条件、预期产物、result summary、recent runs compare / filter、last-success，以及 artifact / stdout / stderr 钻取状态
