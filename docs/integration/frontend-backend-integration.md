...
- author studio 的 isolated workspace query 和 job 驱动刷新
- docs 上下文面板
...
- `/api/registry/remote/index`
- `/api/registry/remote/channels/[channel]`
- `/api/registry/remote/skills/[skillId]`
- `/api/registry/remote/bundles/[bundleId]`
...
- `/studio` 三个页面都支持可选 `submissions_root` / `inbox_root` query，用于隔离 author workspace、演示目录和 E2E 测试目录。
...
- author studio 页面当前使用 `force-dynamic`，author overview / submissions 数据在 API 模式下使用 `no-store`，避免 job 执行后继续读到缓存快照。
- 当前已经完成 author API 和页面骨架，后续剩余的是更细的权限、凭证和 hosted publish UI。
...
- 这轮只补到了 backend browse API、Next.js 代理层和 CLI browse 命令；前端 `/registries/*` 页面当前仍未落地。
