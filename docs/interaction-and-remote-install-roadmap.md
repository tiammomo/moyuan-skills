...
- docs 页面已经有搜索、过滤、上下文、相关文章、action panel、allowlist backend action execution，以及 recent runs / last-success 回看与 artifact / stdout / stderr drilldown
- 中文教学与业务案例文档已经接入
...
当前最大的缺口已经从“怎样把复盘细节和工件钻取做深”进一步转成了“怎样把多次运行结果做成更高效的比较与筛选”：

1. docs action 现在已经能回看 artifact / stdout / stderr，但 recent runs 之间还缺少更高效的 compare / filter / 差异提示
2. governance approval audit 目前还没有更细的 archive 检索、筛选与批量回看能力
3. remote install 还可以继续增强 org policy / provenance 级别的组合解释

## 下一轮建议

下一轮优先建议继续顺着 docs action 往下做 recent runs compare / filter，而不是立刻回到更重的治理 UI。

原因：

- docs action execution、recent runs 和 artifact drilldown 已经有了，现在最值得补的是“怎样在多次运行之间更高效地比较和筛选结果”
...
- docs action recent runs 的 compare / filter / 状态聚合
- result summary、artifact drilldown 与历史选择之间的联动筛选
- Playwright 覆盖 docs action compare / filter / revisit 的真实链路

下一轮规划文档见 [frontend-docs-action-compare-filter-iteration.md](./frontend-docs-action-compare-filter-iteration.md)。
