...
当前前端已经能直接消费真实后端数据的页面包括首页、skills、channels、bundles、docs，以及 `docs/teaching` 和 `docs/project` 的列表/详情页；同时 docs center 也已经支持跨三类文档的统一搜索和筛选，详情页还补上了继续阅读的 related navigation、doc-specific context panels，以及可直接执行的 action panels。
