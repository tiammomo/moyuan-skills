...
这一轮把 docs action 再往前推进了一步，从“能解释选中运行和基线成功的差异”补到了“能顺着差异提示直接打开最值得先看的 drilldown”：

- docs action 现在除了 recent runs compare / filter 和 run diff summary，还会给出 artifact / stdout / stderr 的 section diff 状态
- result summary 会直接给出 quick-open 入口，把用户带到建议优先排查的 drilldown
- artifact / stdout / stderr 卡片会区分 changed、stable、baseline、empty 等状态，并高亮当前最值得先看的区块
- docs action history 仍然通过安全的 `doc kind + doc id + action id` 映射读取，前端只消费 allowlist action 的最近记录
- Playwright 已覆盖 docs action 的 diff summary、quick-open handoff、失败输出复盘，以及刷新后的 last-success revisit
...
- docs 详情页现在会把 repo 命令、安全执行状态、recent runs、compare / filter、run diff summary、section diff 状态、quick-open handoff、last-success、artifact/stdout/stderr drilldown、顺序提示、前置条件、预期结果和产物提示一起展示，并为 allowlist 动作提供页内执行入口
